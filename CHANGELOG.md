...
- **Zone-composition preview cards** — Redesigned card family (`ZoneCard`) with a full-width top thumbnail zone. `AssetCard` renders a real per-format preview via the AssetViewer thumbnail dispatch; `SlotCard`/`PackCard`/`TemplateCard` use status/entity glyphs by design. Card zones (header, content, status, actions) compose from discrete slot components, enabling straightforward card variants (ADR-3, D-012).
