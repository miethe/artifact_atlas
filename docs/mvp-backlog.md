...
| DI-SubjectCollapse | `(route, subject)` identity collapses non-dossier reports | The M3 stable-id key is `(route, subject)`, but `subject` is a project/feature **slug**, not a per-instance key — so `phase`/`program`/`readiness` reports for one project overwrite each other. The missing discriminator is upstream (DI-283 / PF-3 OQ-5). Route-gating the stable-id path to `{feature, dossier}` would fix the silent overwrite locally, but needs an explicit amendment to the accepted OQ-3 decision. Confirmed by regression test, not fixed | PF-3 OQ-5 settling the discriminator, or an OQ-3 amendment | Deferred, documented |
