...
|---------|-------|-----------------|----------------------|--------|
| DI-G4 | Cross-scope Reports lens | `GET /api/reports` is the necessary additive contract for a server-faceted cross-project lens; full epic hierarchy is not inferred. V1 facets actual `intenttree_node` links as the tracker-node proxy while DI-G6/DI-LinkTarget remain blockers. | First-class epic taxonomy and approved IntentTree read policy | 🚧 IMPLEMENTED FOR REVIEW in PF-4 sprint |
...
| DI-R7 | Fleet-wide backfill of scattered `.claude/reports/…` HTML | The dry-run-first backfill imported the 14 manifest-backed AOS Atlas prototype reports into the canonical registry without changing their source files. | — | **Resolved 2026-09-09** (D-019 §10–11; PF4-3) |
| DI-sensitivity | Report-asset sensitivity defaulting posture | Ingest takes an explicit `--sensitivity` if given, else the workspace default, **floored at non-public** (`ImportService._report_sensitivity`). Reports carry operator-internal detail (commit hashes, paths, model routing), so `public` is unreachable by default. What remains open is *policy guidance*, not mechanism: whether reports should default stricter than the workspace baseline (e.g. `work_sensitive`) | Security review / operator posture confirmation after pilot | Deferred, pending confirmation |
| DI-SubjectCollapse | Report identity now `(route, subject, instance_key)` | PF-3 resolved OQ-5 (2026-08-02, `agentic_meta_dev` #87) by adding a named `instance_key` to the export envelope. Atlas extends its identity key to `(route, subject, instance_key)` instead of route-gating: distinct instances get distinct assets, re-publishing one instance still replaces in place, and no OQ-3 amendment was needed. Recurring route with no `instance_key` → create, never overwrite | — | **Resolved 2026-08-03** (see D-018 / DI row in `docs/DECISIONS.md`) |
...
| PF4-2 | Fleet Atlas project rows | `scripts/seed_fleet_projects.py` seeds a project row per AOS repo from `agentic_meta_dev/docs/05-app-registry.yaml`; dry-run default, deterministic `proj_<slug>` ids, one shared workspace, tombstone-aware uniqueness on both id and slug, `--allow-real-registry` gate before touching canonical state. Verified: 42 apps → 41 create / 1 skip / 0 problem. | ✅ SHIPPED 2026-08-08 (D-019 §7–9); **APPLIED to the live node 2026-08-08** at the PF4-2b scope (25 of 42 apps → 24 create / 1 skip, `projects.jsonl` 1 → 25 rows, all `ws_artifact_atlas_local`, served by `GET /api/projects`). Also **applied to this checkout's canonical `registry/`** 2026-08-11 (D-021, `node_01KZRMQ1RS9N7SDCXNDGXVY16Y`): `--active-since 14 --apply --allow-real-registry`, 42 → 24 create / 1 skip / 9 stale / 8 unresolved, `projects.jsonl` 1 → 25 rows, all `ws_artifact_atlas_local`, idempotent on re-run |
| PF4-2b | Fleet seeding scope (commit recency) | Resolves D-019's open scoping question (`node_01KZHNGY48EVDTR4G814048QNX`): seed by **commit recency**, not declared layer (static) or report history (circular). `--active-since DAYS` measures each fleet app's last commit via its registry `path:`; `STALE` (measured, too old) and `UNRESOLVED` (unmeasurable) are separate reported outcomes, never collapsed, neither trips `--strict`, and neither reserves a slug/id. Handles a fleet path that is a repo **subdirectory** (`hermes` inside `agentic_meta_dev`) via `git log -1 -- <relpath>`. Defaults off, so the D-019 plan stays byte-identical. Verified at 14d: 42 → 24 create / 1 skip / 9 stale / 8 unresolved. | ✅ SHIPPED 2026-08-08 (D-020) |
| PF4-3 | DI-Backfill report ingest | `scripts/backfill_reports.py` — dry-run-first, explicit selection, reuses `ImportService.import_report`, synthesizes envelopes from each report's own manifest, never moves/rewrites a canonical file, anchors `instance_key` explicitly and records its derivation. Verified: 15 candidates → 14 ingestable. Managed blob URIs are workspace-relative, and unchanged re-ingest repairs a missing runtime blob. Resolves **DI-R7** above. | ✅ SHIPPED 2026-08-08 (D-019 §10–11); **APPLIED to canonical registry 2026-09-09**: 14/14 ingestable prototype reports, 13 project-attributed and one centrally discoverable unattributed report; post-merge deployed-volume activation remains a steward step because content blobs are intentionally gitignored |
| PF4-4 | Per-project Reports surface (operator ask #1) | Reports-only lens on the project page with route / revision / truth_status / source commit, rows opening the hosted HTML, and linked tracker nodes. | 🚧 IMPLEMENTED FOR REVIEW in PF-4 sprint. `docs/project_plans/implementation_plans/features/reports-hub-per-project-surface-v1.md` |
| PF4-5 | Cross-project `/reports` lens (operator ask #2) | `GET /api/reports` and the top-level surface list reports across projects with server-owned project / route / truth_status / date / tracker-node facets. Actual `intenttree_node` links are the v1 epic proxy; DI-G6/DI-LinkTarget remain the blocker to hierarchy semantics. | 🚧 IMPLEMENTED FOR REVIEW in PF-4 sprint; additive API correction recorded in D-022 |
| PF4-6 | Dynamic AOS-wide overview (operator ask #3) | Laptop collector emits a typed, secret-screened `fleet_snapshot`; Atlas persists it and serves a zero-live-call `GET /api/overview` with provenance-bearing metrics and newest program/dossier report links. The 2026-09-08 remeasurement, 2026-09-09 14-project activation snapshot, and `:8099` retirement split are recorded in D-022. | 🚧 IMPLEMENTED FOR REVIEW — backend/collector/docs and overview UI landed in the PF-4 sprint; upstream static-server removal remains coordinated follow-up |
