---
title: "Repository Architecture (Hexagonal Pattern)"
description: "Defines hexagonal architecture with abstract repository interfaces, DI routing, and implementation patterns for local and enterprise editions."
type: architecture
domain: backend
status: active
last_verified: 2026-03-12
load_when: "When working on data models, repositories, or database operations"
---
...
| File | Purpose |
|------|---------|
| `repositories.py` | 14 ABC interfaces: `IArtifactRepository`, `IProjectRepository`, `ICollectionRepository`, `IDeploymentRepository`, `ITagRepository`, `ISettingsRepository`, `IGroupRepository`, `IContextEntityRepository`, `IMarketplaceSourceRepository`, `IProjectTemplateRepository`, `IToolGraphRepository`, `IRawMetadataRepository`, `IToolGraphRepository` (enterprise), `IRawMetadataRepository` (enterprise) |
...
| `local_tool_graph.py` | IToolGraphRepository | SQLAlchemy cache only (tool definitions, artifact-tool declarations, tool review) |
| `local_raw_metadata.py` | IRawMetadataRepository | SQLAlchemy cache only (raw frontmatter preservation per artifact version) |
...
| `enterprise_tool_graph.py` | IToolGraphRepository | Full CRUD with tenant-filtered SELECT + JSONB columns for tool definitions and declarations |
| `enterprise_raw_metadata.py` | IRawMetadataRepository | Full CRUD with tenant-filtered SELECT + raw_json JSONB for artifact version raw frontmatter |
...
### Enterprise Write-Once Contract (WS-2)

`EnterpriseFileContentService` follows a write-once contract for artifact versions: mutations create new version rows (append-only invariant), never update existing ones. The `change_origin="stub"` field serves as the draft discriminator for in-flight changes. See `skillmeat/core/services/enterprise_file_content.py` for storage patterns.

### Deployment Cache Append-Only (WS-2)

`EnterpriseDeploymentRepository.sync_deployment_cache()` enforces insert-only semantics for `ProjectVersionDeployment` rows. All writes are append-only (new version records), preserving full audit trail. See `skillmeat/cache/enterprise_repositories.py::EnterpriseDeploymentRepository` for upsert implementation.
