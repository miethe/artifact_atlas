...
| delegation-router | 2.0 | active (repo-scope) | ✗ | 6 | Repo-scope resolver: routes (model, provider, effort, profile, task_class) → RoutingRecord. Includes audit-log writer and `skillmeat routing audit` CLI. Shipped 2026-06-09. |
...
| skillmeat-cli | 1.4.0 | active | ✓ | 20 | SkillMeat CLI orchestration & artifact management (v0.54+: real bundle export/import, marketplace install, deployment-sets, context-entities, groups, universal versioning). |
...
| workflow-authoring | 0.1.0 | active | ✗ | 2 | Governed procedure for authoring SkillMeat Dynamic Workflow scripts (.claude/workflows/*.js). |

## Notes

- **No Deprecations**: All 58 skills are currently in `active` status.
- **delegation-router Promotion (2026-06-09)**: `delegation-router` v2.0 promoted from user-scope skeleton to repo-scope full resolver engine (RoutingRecord emission, append-only audit log, `skillmeat routing audit` CLI, 4 agentType wrappers). Part of `delegation-router-multimodel-v1` DeploymentSet. SPEC.md not yet created.
- **WS-6 Refactor (2026-06-02)**: Web controller decomposition (WS-6) introduces new hook domain structure (`hooks/{marketplace,sync,discover,context}/`) but does not change `frontend-design` skill domain coverage — it remains focused on production-grade frontend interface creation. No skills-index update required.
